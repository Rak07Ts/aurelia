...
export const ThemeSwitcher: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  const themes: { id: ThemeMode; label: string; icon: React.ReactNode; previewBg: string }[] = [
    { id: 'default', label: 'Editorial Light', icon: <Sun size={15} />, previewBg: '#F8F6F1' },
    { id: 'immersive_dark', label: 'Immersive Dark', icon: <Moon size={15} />, previewBg: '#171916' },
    { id: 'earth_retreat', label: 'Earth Retreat', icon: <Mountain size={15} />, previewBg: '#ECE9DF' },
  ];
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 rounded-[2px] border border-border-subtle hover:border-accent-primary bg-surface-primary text-text-primary text-body-s transition-all cursor-pointer"
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 p-2 rounded-[2px] bg-surface-primary border border-border-default shadow-large z-50 animate-in fade-in slide-in-from-top-1">
          <p className="px-2 py-1 text-[10px] font-medium uppercase tracking-uppercase text-text-muted">
            Atmospheric Theme
          </p>
